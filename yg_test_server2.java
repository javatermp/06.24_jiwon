...
public class test_server2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			ServerSocket s = new ServerSocket(8199); // 앞의 문제와 다르게 포트 8199로 생성한다. 그리고 서버에 접속할 때마다 새로운 시도를 하게 한다.			
			for (;;) {
				Socket incoming = s.accept();
				Runnable task = new ThreadedEchoHandler3(incoming, i); // Runnable과 관련된 클래스 객체 생성
...
class ThreadedEchoHandler3 implements Runnable {
	public ThreadedEchoHandler3(Socket i, int c) {
		incoming = i;
		num = c;
	}
...
//				int number = Integer.parseInt(str);
				...
				if(str == null) {
					...
					System.out.println(number + ", " + ck + " : " + num + " >> server2");
